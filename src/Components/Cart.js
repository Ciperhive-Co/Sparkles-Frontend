...
const Cart = ({products = []}) => {
    ...
    const handlePrev = () => {
      ...
      const handleIncrement = (value, index) => {
        ...
        updatedBPriceArray[index] = parseFloat(newBPrice.toFixed(productPriceBefore.toString().split('.')[1]?.length || 0));
        ...
        updatedAPriceArray[index] = parseFloat(newAPrice.toFixed(productPriceAfter.toString().split('.')[1]?.length || 0));
        setAPriceArray(updatedAPriceArray);
      
        // Update the total based on updatedQuantityArray
        const calculatedTotal = updatedAPriceArray.reduce((acc, price) => acc + price, 0).toFixed(
          Math.max(...updatedAPriceArray.map((value) => value.toString().split('.')[1]?.length || 0))
        );
      ...
      useEffect(() => {
        ...
        calculatedTotal = calculatedTotal.toFixed(
          Math.max(
            ...initialBPrices.map((price) => price.toString().split('.')[1]?.length || 0),
            ...initialAPrices.map((price) => price.toString().split('.')[1]?.length || 0)
          )
        );
  ...
  return (
                    ...
                    <div className="cart-item-content">
                        ...
                        <div className="productpricequantity">
                            <div className="productprice">
                                <div className="product-price1">${product.priceAfter} each </div>
                                <div className="product-price2">${product.priceBefore}</div>
                            </div>
                            <div className="quantityRecord">
                                <div className="round-button">
                                <button className="minusButton" onClick={() => handleDecrement(product.priceAfter, index)}><span className="plus-minus-sign">-</span> </button>
    ...
    <div className="cartproductCard">
        ...
        {dummyproducts.slice(currentIndex, currentIndex + 4).map((product, index) => (
